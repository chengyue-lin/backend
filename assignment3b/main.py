...
CLOUD_STORAGE_BUCKET = "book-library-api"
...
@app.route("/")
def homepage():
    """datastore_client = datastore.Client.from_service_account_json('book-library-api-348006-05d7a8fcea1b.json')
    ...
    return "Hello Book Library API!", 200
...
def books():
    if request.method == 'GET':
        ...
        if request.args.get('author') is not None and request.args.get('language') is not None:
            ...
            query = datastore_client.query(kind='Books')
            if request.args.get('cursor') is None:  # also, want to ask is the cursor work correctly?
                cursor = None
            else:
                cursor = request.args.get('cursor')
            query_iter = query.fetch(start_cursor=cursor, limit=10)
    ...
    if request.method == 'POST':
        ...
        title = data['title']
        year = data['year']
        storage_client = storage.Client.from_service_account_json('book-library-api-348006-05d7a8fcea1b.json')
...
@app.route("/books/<isbn>", methods=['GET', 'POST', 'PUT', 'DELETE'])
def operation(isbn):
    if request.method == 'GET':
